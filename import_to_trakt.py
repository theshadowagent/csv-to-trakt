...
class TraktImporter:
    ...
    def pickle_token(self, access_token):
        with open('trakt_access_token.pickle', 'wb') as f:
            pickle.dump(access_token, f)

    def unpickle_token(self):
        try:
            with open('trakt_access_token.pickle', 'rb') as f:
    ...
    def get_trakt_session(self):
        ...
        if not access_token:
            resp = requests.post('https://api.trakt.tv/oauth/device/code', json={'client_id': CLIENT_ID}).json()

            print(f"Enter {resp['user_code']} in the opened Trakt page in your browser to authorize the import")
            print("(you can revoke the access anytime on https://trakt.tv/oauth/authorized_applications)")
            time.sleep(2)
            ...
            for i in range(int(expires_in / interval)):
                r = requests.post('https://api.trakt.tv/oauth/device/token', data={
                    'client_id': CLIENT_ID,
    ...
    def import_episodes(self):
        ...
        for name in show_names:
            ...
            for season in seasons:
                try:
                    episodes_rows = shows_df[shows_df['Name'] == name][shows_df['Season'] == season]
                    show['seasons'].append({
                        'number': int(season),
                        'episodes': [
                            {'watched_at': row['Date Watched'],
                             'number': row['Episode']} for _, row in episodes_rows.iterrows()
                        ]
                    })
                except ValueError:
                    continue
            shows.append(show)
        r = self.session.post('https://api.trakt.tv/sync/history', json={
            'shows': shows
        }).json()
        print(f"Imported {r['added']['episodes']} / {len(shows)}")
        print(f"Not found {len(r['not_found']['shows'])} / {len(shows)}")
    ...
    def import_movies(self):
        ...
        print(f"Imported {r['added']['movies']} / {len(movies)}")
    ...
    def fix_not_found_movies(self, movies_df, trakt_response):
        ...
        nf_watched_dates = list(map(lambda x: x['watched_at'].split('T')[0], trakt_response['not_found']['movies']))
        nf_movies_df = movies_df[movies_df['Date Began'].isin(nf_watched_dates)]
        nf_titles = nf_movies_df['Movie Title'].values
        trakt_id_map = self.find_movies_on_trakt(nf_titles,
                                            nf_movies_df['Release Date'].values)
        ...
        print(f"Fixed {r['added']['movies']} / {len(nf_movies)}")

    def find_movies_on_trakt(self, not_found_movies, release_years):
        trakt_ids_map = {}
        for idx, nf_movie in tqdm(enumerate(not_found_movies)):
            res = self.session.get(f'https://api.trakt.tv/search/movie?query={nf_movie}').json()
            res = list(filter(lambda x: not release_years[idx] or
                                        x['movie']['year'] and abs(x['movie']['year'] - release_years[idx]) < 2, res))
